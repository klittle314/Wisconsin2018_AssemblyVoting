...
#build dataframe with numbers of votes
...
test55 <- df_spread[df_spread$winner=="R",]
test66 <- df_spread[df_spread$winner=="D",]

debugonce(df_data_maker)
test <- df_data_maker(i=1)
...
test222 <- expand.grid(names_set2, as.character(1:99))
...
df_OfficeTotal <- df1[df1$X__1==OfficeTotal,]
votes <- df_OfficeTotal[!(rowSums(is.na(df_OfficeTotal))==NCOL(df_OfficeTotal)),] 
names(votes)[1] <- "district"


create_df_row <- function(df) {
  #get string of 
  
  df_OfficeTotal <- df1[df1$X__1==OfficeTotal,]
  test4 <- df_OfficeTotal[!(rowSums(is.na(df_OfficeTotal))==NCOL(df_OfficeTotal)),] 
}

df2 <- read_excel("Ward by Ward Report-Gen Election-Assembly.xlsx", 
                  sheet = "Sheet3", range = "A7:J100")

df3 <- read_excel("Ward by Ward Report-Gen Election-Assembly.xlsx", 
                  sheet = "Sheet4", range = "A7:J100")